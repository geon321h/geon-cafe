			...
			$('#idCheck').click(function(){
				...
				$.ajax({
					...
					success : function(data) {
						if($('input[name="id"]').val()=='${mb.id}'){
							$('#idMessage').html("기존 아이디입니다.").css('color','green');
							possible = true;
	...
	<input type="hidden" name="num" value="${mb.num}">
		...
		<table border=1>
			<tr>
				<td>아이디</td>
				<td>
					<input type="text" name="id" size="15" value="${mb.id}">
			...
			<tr>
				<td>이름</td>
				<td><input type="text" name="name" size="15" value="${mb.name}">
			</tr>
			<tr>
				<td>나이</td>
				<td><input type="text" name="age" size="15" value="${mb.age}">
			</tr>
			<tr>
				<td>좋아하는 장르</td>
				<td> 
				...
				<td>
					<select name="time" >
						<c:forEach var="time" items="<%=time %>">
							<option value="${time }"  <c:if test="${time==mb.time}"> selected </c:if>>${time }</option>
						</c:forEach>
					</select>
				</td>
			</tr>
			...
			<tr>
				<td>개선사항</td>
				<td>
					<textarea name="memo" rows="6" placeholder="value는 여기에 넣기">${mb.memo}</textarea>
		...
		<br> 
		<input type="submit" value="수정하기" id="insertSub">
	</form>
